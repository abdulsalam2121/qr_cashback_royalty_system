...
router.post('/:tenantSlug/billing/subscribe', resolveTenant, auth, rbac(['tenant_admin']), validate(subscribeSchema), asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if ((plan as any).cardAllowance > 0) {
        await tx.tenant.update({
          where: { id: tenant.id },
          data: {
            subscriptionCardLimit: (plan as any).cardAllowance,
            totalCardAllowance: { increment: (plan as any).cardAllowance },
            currentCardBalance: { increment: (plan as any).cardAllowance }
          } as any
        });
      }
    });
