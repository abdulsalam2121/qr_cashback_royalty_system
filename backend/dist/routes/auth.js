...
router.post('/sync', verifyFirebaseToken, asyncHandler(async (req, res) => {
    ...
    try {
            ...
            if (user.role === 'customer' && user.authProvider !== 'google') {
                ...
                user = await prisma.user.update({
                    ...
                    data: {
                        ...
                        tenantId: tenant.id,
                        lastLogin: new Date(),
                        displayName: firebaseUser.name || firebaseUser.displayName || user.displayName || `${user.firstName || ''} ${user.lastName || ''}`.trim() || null,
                        photoURL: firebaseUser.picture || firebaseUser.photoURL || user.photoURL || null,
                        firstName: firstName,
                        lastName: lastName,
                    },
                    include: {
        ...
        const response = { user: userResponse, role };
        // Include tenant info for non-platform admins
        if (user.tenant && user.role !== 'platform_admin') {
            response.tenant = {
                id: user.tenant.id,
